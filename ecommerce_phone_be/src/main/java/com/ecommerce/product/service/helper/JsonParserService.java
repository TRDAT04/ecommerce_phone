...
public class JsonParserService {
    ...
    public List<VariantDTO> parseVariants(String json) {
        try {
            return mapper.readValue(
                    json,
                    mapper.getTypeFactory().constructCollectionType(List.class, VariantDTO.class)
            );
        } catch (IOException e) {
    ...
    public List<SpecDTO> parseSpecs(String json) {
        try {
            return mapper.readValue(
                    json,
                    mapper.getTypeFactory().constructCollectionType(List.class, SpecDTO.class)
            );
        } catch (IOException e) {
            throw new RuntimeException("Invalid specs JSON", e);
        }
    }
    ...
    public List<ColorDTO> parseColors(String json) {
        try {
            ...
            return mapper.readValue(
                    json,
                    mapper.getTypeFactory().constructCollectionType(List.class, ColorDTO.class)
            );
        } catch (IOException e) {
            throw new RuntimeException("Invalid colors JSON", e);
        }
    }
}
