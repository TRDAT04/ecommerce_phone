...
public class EmailTemplateBuilder {
    ...
    // ─── Template: Admin xác nhận đơn hàng ──────────────────────────────────────
    public String buildAdminConfirmedHtml(Order order) {
        NumberFormat currencyFormat = NumberFormat.getInstance(new Locale("vi", "VN"));
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm - dd/MM/yyyy");

        String orderDate = order.getCreatedAt() != null ? order.getCreatedAt().format(dtf) : "---";
                      ...
                      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
                        ...
                        <tr>
                          <td style="background:linear-gradient(135deg,#1a1a2e 0%%,#16213e 50%%,#0f3460 100%%);padding:36px 40px;text-align:center;">
                            <div style="font-size:32px;margin-bottom:8px;">📱</div>
                            <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:0.5px;">NextMobile</h1>
                            <p style="margin:8px 0 0;color:rgba(255,255,255,0.75);font-size:14px;">Cập nhật trạng thái đơn hàng</p>
                          </td>
                        </tr>
                        ...
                        <tr>
                          <td style="background:#eff6ff;padding:20px 40px;text-align:center;border-bottom:1px solid #bfdbfe;">
                            <span style="font-size:28px;">🎉</span>
                            <h2 style="margin:8px 0 4px;color:#1d4ed8;font-size:18px;font-weight:600;">Đơn hàng đã được xác nhận!</h2>
                            <p style="margin:0;color:#555;font-size:14px;">Admin đã xác nhận đơn hàng của bạn. Chúng tôi sẽ sớm tiến hành giao hàng.</p>
                          </td>
                        </tr>
                            ...
                            <table width="100%%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
                              ...
                              <tr>
                                <td colspan="2" style="padding:16px 20px;">
                                  <span style="color:#888;font-size:13px;">Trạng thái</span>
                                  <div style="margin-top:4px;">
                                    <span style="background:#dbeafe;color:#1d4ed8;font-size:13px;font-weight:600;padding:4px 12px;border-radius:20px;">✅ Đã xác nhận</span>
                                  </div>
                                </td>
                              </tr>
                            </table>
                        ...
                        <tr>
                          <td style="padding:24px 40px 32px;">
                            <div style="background:#eff6ff;border-radius:10px;padding:16px 20px;border-left:4px solid #3b82f6;">
                              <p style="margin:0;color:#1d4ed8;font-size:13px;line-height:1.6;">
                                🚚 <strong>Lưu ý:</strong> Đơn hàng của bạn đã được xác nhận và đang được chuẩn bị.
    ...
    public String buildCancelledHtml(Order order) {
        NumberFormat currencyFormat = NumberFormat.getInstance(new Locale("vi", "VN"));
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm - dd/MM/yyyy");
        ...
        if (order.getOrderDetails() != null) {
            for (OrderDetail detail : order.getOrderDetails()) {
                ...
                itemsHtml.append("""
                        <tr>
                          <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;">
                            <div style="font-weight:500;color:#6b7280;">%s%s</div>
                          </td>
                          <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;text-align:center;color:#9ca3af;">x%d</td>
                          <td style="padding:10px 12px;border-bottom:1px solid #f0f0f0;text-align:right;color:#9ca3af;">%s₫</td>
                        </tr>
                        """.formatted(productName, variantInfo, detail.getQuantity(), currencyFormat.format(lineTotal)));
            }
        }
                      ...
                      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
                        ...
                        <tr>
                          <td style="background:linear-gradient(135deg,#1a1a2e 0%%,#16213e 50%%,#0f3460 100%%);padding:36px 40px;text-align:center;">
                            <div style="font-size:32px;margin-bottom:8px;">📱</div>
                            <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:0.5px;">NextMobile</h1>
                            <p style="margin:8px 0 0;color:rgba(255,255,255,0.75);font-size:14px;">Thông báo đơn hàng</p>
                          </td>
                        </tr>
                        ...
                        <tr>
                          <td style="background:#fef2f2;padding:20px 40px;text-align:center;border-bottom:1px solid #fecaca;">
                            <span style="font-size:28px;">❌</span>
                            <h2 style="margin:8px 0 4px;color:#dc2626;font-size:18px;font-weight:600;">Đơn hàng đã bị hủy</h2>
                            <p style="margin:0;color:#555;font-size:14px;">Đơn hàng của bạn đã được hủy thành công. Hàng tồn kho đã được hoàn trả.</p>
                          </td>
                        </tr>
                            ...
                            <table width="100%%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
                              ...
                              <tr>
                                <td colspan="2" style="padding:16px 20px;">
                                  <span style="color:#888;font-size:13px;">Trạng thái</span>
                                  <div style="margin-top:4px;">
                                    <span style="background:#fee2e2;color:#dc2626;font-size:13px;font-weight:600;padding:4px 12px;border-radius:20px;">❌ Đã hủy</span>
                                  </div>
                                </td>
                              </tr>
                            </table>
                        ...
                        <tr>
                          <td style="padding:16px 40px 0;">
                            <table width="100%%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;">
                              <tr>
                                <td style="padding:16px 20px;">
                                  <span style="color:#9ca3af;font-size:15px;font-weight:500;">Tổng đơn hàng</span>
                                </td>
                                <td style="padding:16px 20px;text-align:right;">
                                  <span style="color:#9ca3af;font-size:20px;font-weight:700;text-decoration:line-through;">%s₫</span>
                                </td>
                              </tr>
                            </table>
                          </td>
                          ...
                          <td style="padding:24px 40px 32px;">
                            <div style="background:#fef2f2;border-radius:10px;padding:16px 20px;border-left:4px solid #ef4444;">
                              <p style="margin:0;color:#dc2626;font-size:13px;line-height:1.6;">
                                💬 Nếu bạn có thắc mắc về đơn hàng này, vui lòng liên hệ hotline hoặc fanpage của chúng tôi để được hỗ trợ.
                              </p>
                            </div>
                          </td>
                        </tr>
